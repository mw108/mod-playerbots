...
bool Aq40MoveToVeknilashTrigger::IsActive()
{
    if (Unit* boss = AI_VALUE2(Unit*, "find target", "emperor vek'nilash"))
    {
        if (boss->IsInCombat())
        {
            ...
            if (botDist < 5.0f)
            {
                bot->TellMaster("Moving to Emperor Vek'nilash", PLAYERBOT_SECURITY_TALK);
                return true;
            }
        }
    }
    ...
    {
        bot->TellMaster("Emperor Vek'nilash not found", PLAYERBOT_SECURITY_TALK);
    }
    return false;
}
...
bool Aq40MoveToVeklorTrigger::IsActive()
{
    if (Unit* boss = AI_VALUE2(Unit*, "find target", "emperor vek'lor"))
    {
        if (boss->IsInCombat())
        {
            ...
            if (botDist < 20.0f)
            {
                bot->TellMaster("Moving to Emperor Vek'lor", PLAYERBOT_SECURITY_TALK);
                return true;
            }
        }
    }
    ...
    {
        bot->TellMaster("Emperor Vek'lor not found", PLAYERBOT_SECURITY_TALK);
    }
    return false;
}
